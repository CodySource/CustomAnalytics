...
namespace CodySource
{
    namespace CustomAnalytics
    {
        ...
        public class AnalyticsFlags : AnalyticsDataComponent
        {
            ...
            /// </summary>
            public void SetFlag(string _pLabel) => table.data[table.data.FindIndex(f => f.label == _pLabel)] = new DataEntry() { label = _pLabel, valueType = EntryValues.Bool, boolValue = true };

            /// <summary>
            /// Sets the respective flag to false
            /// </summary>
            public void ResetFlag(string _pLabel) => table.data[table.data.FindIndex(f => f.label == _pLabel)] = new DataEntry() { label = _pLabel, valueType = EntryValues.Bool, boolValue = false };
